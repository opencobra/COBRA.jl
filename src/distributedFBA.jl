...
function preFBA!(model, solver, optPercentage::Float64=0.0, osenseStr::String="max",
    ...
    if hasObjective
        if printLevel > 0
            printstyled("preFBA! [osenseStr = $osenseStr]: FBAobj = $FBAobj, optPercentage = $optPercentage,
            objValue = optPercentage * FBAobj = $objValue, norm(fbaSol) = $(LinearAlgebra.norm(fbaSol)).\n\n"; color=:blue)
        end
...
function distributedFBA(model, solver; nWorkers::Int=1, optPercentage::Union{Int64, Float64}=0.0, objective::String="max",
