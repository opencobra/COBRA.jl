...
if [ "$ARCH" == "Linux" ]; then
    if [ "$JULIA_VER" == "v0.6.3" ]; then
        ...
        # adding coverage
        /mnt/prince-data/JULIA/$JULIA_VER/bin/julia --color=yes -e 'using Coverage; Codecov.submit_generic(process_folder(), ci="artenolis", commit=ENV["GIT_COMMIT"], branch=ENV["GIT_BRANCH"]);'
